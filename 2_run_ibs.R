...
#	genotypes <- gsub("SNP.csv$", "variety-info.csv", f) |> data.table::fread() |> data.frame()
	fgeno <- file.path("output/reference", gsub("SNP.csv$", "variety-info-refined.csv", basename(f)))
...
#MAF_cutoff=0.05; SNP_Missing_Rate=0.2; Ref_Missing_Rate=0.2; Sample_Missing_Rate=0.2; Ref_Heterozygosity_Rate = 0.1; Sample_Heterozygosity_Rate=0.1; IBS_cutoff=0.5; Inb_method = "mom.visscher"; assign_threshold=.9, threads=1; verbose=FALSE; filename=""
