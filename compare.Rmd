...
```{r pre_confusion}
q <- lapply(d1, \(x) {
    f <- as.data.frame(table(x$variety.x, x$variety.y))
    colnames(f) <- c("IBS", "DAP", "n")
    f <- f[rev(order(f$n)), ]
...
For which varieties is there most agreement?
...
qq <- qq[qq[,1] == qq[,2], ]
cap <- paste0("varieties (", olabs[1] ,")")
kableExtra::kbl(qq, caption=cap) |> kableExtra::kable_classic(full_width = F,  position = "right") |> kableExtra::kable_styling(bootstrap_options = c("striped", "hover"))  |> kableExtra::scroll_box(width = "100%", height = "400px")
```
</div>
<div class = "col-md-5">
```{r tab4a}
...
For what varieties is there most confusing?
...
rank_plot <- function(z, add=FALSE, main="") {
	...
	if (add) {
		...
		legend("bottomright", legend=paste(rev(M), "#1, ranked by", M), col=c("blue", "red"), lwd=2, cex=.75)
	} else {
