...
```{r vars, fig.width=4.5, fig.height=5}
...
add <- bm[1, ]
add[1,1] <- "not matched"
add[1,2] <- nrow(bibs) - sum(bibs$IBS >= 0.8)
add[1,3] <- 100 - p8
bm = rbind(bm, add)
bm = bm[rev(order(bm[,2])), ]
...
```{r plotmatch, fig.height=3.5, fig.width=4}
lab <- as.character(bm[,1])
lab[bm[,2] < 15] <- ""

par(mar=c(4, 4, 0, 1), mfrow=c(1,2))
...
j <- match(colnames(dibs), ibs$field_id)
vars <- ibs$variety[j]
score <- ibs$IBS[j] |> round(2)
k <- is.na(score)
score[k] <- ""
