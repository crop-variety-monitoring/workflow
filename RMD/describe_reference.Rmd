...
pdftxt <- ifelse(is_html, paste0('<a href="', country, "_", crop, "_reference_", method, '.pdf">pdf version</a>'), "")
```
...
This document describes the reference samples for `r crop` in `r country` (order `r ordnr`) using IBS. There were `r sum(inf$reference)` samples `r ifelse(removed > 0, paste0("(after removing ", removed, " sample(s) for having more than 50% missing data)"), "")`, representing `r nvars` varieties. On average, there were 
...
Purity is the fraction of clusters with one variety. Unison is the fraction of varieties that are in a single cluster. The gray line shows at which IBS threshold their mean is highest, and the mean value as in parenthesis (ideally this value would be 1). This may be an estimate of the optional threshold for assignment of variety names to field samples. 
...
plotpu(x, "Before cleaning")
plotpu(y, "After cleaning")
```
...
boxplot(nn, ylab="Distance to nearest other variety\n(IBS)", las=1, col="yellow", cex=.5)
