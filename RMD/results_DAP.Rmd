...
```{r gatherref}
ibs8 <- bibs[bibs$DAP > 0.8,]

ibsthr <- data.frame(
		crp=c("Ri", "Er", "Co", "Mz", "Cp", "Ca"),
 	  thr=c(.05,  .07,  .03,   .08,  .05,  .1))

crpth <- 1 - ibsthr$thr[ibsthr$crp==crp]
ibst <- bibs[bibs$DAP > crpth,]
...
scrolltab(bm8, caption=paste("DAP", GE, "0.8"))
```
</div>
<div class = "col-md-6">
```{r tab2}
bmt <- bmt[rev(order(bmt[,2])), ]
scrolltab(bmt, caption=paste("DAP", GE, crpth))
```
</div></div>
...
plot(x, sort(bibs$DAP), ylab="DAP score", xlab="Field samples matched (fraction)", pch=20, col="red", las=1, cex=.5)
grid()
...
legend("topright", legend=c(paste("DAP", GE, "0.8"), paste("DAP", GE, crpth)), col=c("red", "blue"), pch=20)
...
Number of groups (varieties) in the field by DAP dissimilarity threshold.

```{r nvars, fig.width=4.5, fig.height=4}
par(mar=c(4, 4, 0, 1))
cuts <- seq(0.01, 0.2, 0.01)
groupn <- sapply(cuts, \(i) matchpoint:::ngroups.dm(fdibs, i))
plot(c(0, cuts), c(ncol(fdibs), groupn), las=1, ylab="Number of groups", xlab="DAP dissimilarity threshold", type="b", col="red", lwd=2)
grid()
```
...
vars <- ibs8$variety[j]
score <- ibs8$DAP[j] |> round(2)
k <- is.na(score)
score[k] <- ""
score[!k] <- paste0("  [", score[!k], "]")

infr <- inf[inf$reference, ]
#j <- match(labs, infr$sample)
j <- match(colnames(dibs), infr$sample)
refs <- infr$variety[j]
isref <- !is.na(refs)
