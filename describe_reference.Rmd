...
qth <- quantile(v1$value)
outh <- min(400, qth[4] + 1.5 * (qth[4] - qth[2]))
splitln <- Inf
if (outh < qth[5]) {
  splitln <- outh
  dst <- matchpoint::split_groups(dst, outh, TRUE)
  v <- matchpoint::self_dist(dst)
  qth <- quantile(v$value)
  outh <- qth[4] + 2 * (qth[4] - qth[2])
...
cut <- max(1, qth[2])
dst <- matchpoint::lump_similar(dst, cut) 

v <- matchpoint::self_dist(dst)
cut <- min(maxcut, max(2, quantile(v$value)[3]))
dst <- matchpoint::lump_similar(dst, cut) 

v <- matchpoint::self_dist(dst)
cut <- min(maxcut, max(3, quantile(v$value)[4]))
dst <- matchpoint::lump_similar(dst, cut) 
...
text(cut, 0, labels=round(cut), adj=0, xpd=TRUE, cex=.5, col="red")
lines(cbind(splitln, c(0, length(variety))), col=rgb(0,0,1,0.5), lwd=2)
text(splitln, 0, labels=round(splitln), adj=1, xpd=TRUE, cex=.5, col="blue")
...
inf2 <- merge(inf, data.frame(sample=dcn, to=colnames(dst)), by="sample")
...
k <- match(colnames(dibs), dcn)
dimnames(dibs) <- list(variety[k], variety[k])
vibs1 <- matchpoint::self_dist(dibs)
dimnames(dibs) <- list(colnames(dst)[k], colnames(dst)[k])
vibs <- matchpoint::self_dist(dibs)
...
After the cleaning suggested above, the median average Hamming distance between samples of the same variety was `r round(median(v$value), 1)` mutations; `r scores[1]` for technical replicates and `r scores[2]` for biological replicates. 

The median average IBS distance between samples of the same variety was `r round(median(vibs$value), 3)`; `r scores[3]` for technical replicates and `r scores[4]` for biological replicates. 

```{r bplot, fig.width=9, fig.height=3.5}
layout(rbind(1:5), c(1,1,0.5,1,1))
par(mar=c(0, 6, 2, 0))
boxplot(v1$value, ylab="Dissimilarity within varieties\n(mutations)", las=1, main="before cleaning", col="red")
boxplot(v$value, ylab="", las=1, main="after cleaning", col="red")
plot(0, type="n", axes=FALSE, xlab="", ylab="")
boxplot(vibs1$value, ylab="Dissimilarity within varieties\n(IBS)", las=1, main="before cleaning", col="blue")
boxplot(vibs$value, ylab="", las=1, main="after cleaning", col="blue")

nn <- matchpoint::nngb_dist(dst)$value
qnn <- round(quantile(nn), 1)

nnibs <- matchpoint::nngb_dist(dibs)
qnnibs <- round(quantile(nnibs$value), 1)
```

The lowest observed distance between two varieties was `r qnn[1]`, the median was `r qnn[3]` and the maximum was `r qnn[5]` mutations. The minimum IBS distance was `r qnnibs[1]` and the median IBS distance was `r qnnibs[3]` and the largest distance between a variety and its nearest genetic neighbor in the reference set was `r qnnibs[5]`

```{r bplot2, fig.width=6, fig.height=3.5}
par(mar=c(0, 6, 2, 2), mfrow=c(1,2))
boxplot(nn, ylab="distance to nearest neighbor\n(mutations)", las=1, col="red", cex=.5)
boxplot(nnibs$value, ylab="distance to nearest neighbor\n(IBS)", las=1, col="blue", cex=.5)
```
